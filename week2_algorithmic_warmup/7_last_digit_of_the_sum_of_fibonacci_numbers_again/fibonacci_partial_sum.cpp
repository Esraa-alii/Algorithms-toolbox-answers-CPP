/******************************************************************************
 *
 * File Name: fibonacci_partial_sum.cpp
 *
 * Description: Answer for problem 7 in week 2 (Algorithm toolbox coursera)
 *
 * Author: Esraa Ali
 *
 *******************************************************************************/
...
long long get_fibonacci_partial_sum_optimaize(long long from, long long to) {
	if (to < from) return 0;
	vector<long> vec(60);
...
int main() {
	long long from, to;
	std::cin >> from >> to;
	std::cout << get_fibonacci_partial_sum_optimaize(from, to) << '\n';
}
